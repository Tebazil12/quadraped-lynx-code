...
% still_tap = still_tap;
still_tap = still_tap*0; %% for some reason dissim profile is upside down with non 0 still_tap
% still_tap_array = [still_tap(:,:,1);still_tap(:,:,2)];
still_tap_array = still_tap;

folder_name= strcat("H:\git\tactile-core\matlab\demos\voronoi_data\", datestr(now,'yyyy-mm-dd_HHMM'),"\");
mkdir(folder_name)
for t = 1:1000; disp(['t=' num2str(t)])
%         sensor.setPins(still_tap_array);
%         sensor.trackCancel()
%         sensor.trackAsync()
        if mod(t,50) ==0
        sensor.delete()
